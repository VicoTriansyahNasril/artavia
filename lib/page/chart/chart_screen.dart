                ...
                LineChartData(
                  lineTouchData: LineTouchData(
                    touchTooltipData: LineTouchTooltipData(
                      getTooltipColor: (_) => colorBackground,
                      getTooltipItems: (touchedSpots) {
                        return touchedSpots.map((LineBarSpot touchedSpot) {
                          return LineTooltipItem(
                            CurrencyService.to.formatWithoutSymbol(touchedSpot.y),
        ...
        child: Container(
          ...
          child: Column(
            children: [
              ...
              Container(width: 36, height: 3, decoration: BoxDecoration(color: colorGrey, borderRadius: BorderRadius.circular(1.5))),
              const SizedBox(height: 16),
              Padding(
              ...
              const Divider(color: colorDivider, height: 32),
              Expanded(
                child: transactions.isEmpty
                    ? const Center(child: Text('Belum ada transaksi', style: TextStyle(color: colorGrey)))
                    : ListView.builder(
                        itemCount: transactions.length,
                        itemBuilder: (context, index) {
                          final tr = transactions[index];
                          return ListTile(
                            title: Text(tr['note'] as String? ?? 'Transaksi', style: const TextStyle(color: colorWhite)),
                            subtitle: Text(tr['date'].toString().split(' ')[0], style: const TextStyle(color: colorGrey, fontSize: 12)),
                            trailing: Text(
                              CurrencyService.to.formatWithoutSymbol(tr['amount'] as num? ?? 0),
                              style: const TextStyle(color: colorExpense, fontWeight: FontWeight.bold),
                            ),
                          );
                        },
                      ),
